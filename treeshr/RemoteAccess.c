...
STATIC_ROUTINE int FindImageSymbol(char *name, void **sym)
{
  ...
  struct descriptor symname = {DESCRIPTOR_HEAD_INI(0, DTYPE_T, CLASS_S, 0)};
...
STATIC_THREADSAFE struct fd_info_struct { int in_use; int socket; int fd; int enhanced;} *FDS = 0;
...
int MDS_IO_OPEN(char *filename, int options, mode_t mode)
{
  ...
  if (hostpart)
    ...
    if ((fd != -1) && ((options & O_CREAT) != 0)) {
      struct descriptor cmd_d={DESCRIPTOR_HEAD_INI(0,DTYPE_T,CLASS_S,0)};
