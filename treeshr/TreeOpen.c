...
static char *GetFname(char *tree, int shot)
{
  int status = 1;
  static char *ans = 0;
  struct descriptor fname = {DESCRIPTOR_HEAD_INI(0, DTYPE_T, CLASS_D, 0)};
  void *arglist[4];
  char expression[128];
  static void *TdiExecute = 0;
  struct descriptor expression_d = {DESCRIPTOR_HEAD_INI(0, DTYPE_T, CLASS_S, 0)};
  if (ans)
  {
    free(ans);
    ans = 0;
...
{
  static struct descriptor ans_dsc={DESCRIPTOR_HEAD_INI(0, DTYPE_T, CLASS_D, 0)};
  int fd;
  char *ans;
