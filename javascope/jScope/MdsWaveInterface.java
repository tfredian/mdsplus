    ...
    extends WaveInterface
{
    ...
    static final int B_shot = 8, B_x_min = 12, B_x_max = 13, B_y_min = 14,
        B_y_max = 15,
        B_title = 16, B_x_label = 10, B_y_label = 11, B_exp = 7, B_event = 17,
        B_default_node = 9, B_update = 0;
    ...
    public String GetUsedShot()
    {
        ...
        switch (GetShotIdx())
        {
            ...
            case 1:
                ...
                break;
            case 2:
                out = ( (jScopeWaveContainer) (wave.getParent())).
                    getMainShotStr();
                break;
        }
        return out;
    ...
    static public String containMainShot(String in_shot, String m_shot)
    {
        ...
        {
            if (m_shot != null)
                out = "[" + in_shot.substring(0, idx) + m_shot +
                    in_shot.substring(idx + 1) + "]";
            else
                out = "[" + in_shot.substring(0, idx) + "[]" +
                    in_shot.substring(idx + 1) + "]";
        }
        return out;
    }
...
*/
        if (UseDefaultShot())
        {
            ...
            if (main_shot_str != null && main_shot_str.length() != 0)
            {
                curr_shots = ( (jScopeWaveContainer) (wave.getParent())).
                    getMainShots();
                if (curr_shots == null)
                    error = "Main Shot evaluation error: " +
                        ( (jScopeWaveContainer) (wave.getParent())).
                        getMainShotError(true);
            }
            else
            {
                ...
                {
                    curr_shots = GetShotArray(containMainShot(def_vals.shot_str,
                        main_shot_str));
                    if (error == null)
                    {
    ...
    public void ToFile(PrintWriter out, String prompt) throws IOException
    {
        ...
        */
        ...
        if (!is_image)
        {
            ...
            WaveInterface.WriteLine(out, prompt + "update_limits: ",
                                    "" + cin_upd_limits);

            if (show_legend)
            {
                WaveInterface.WriteLine(out, prompt + "legend: ",
                                        "(" + legend_x + "," + legend_y + ")");
            }
        }
        else
        {
            ...
            WaveInterface.WriteLine(out, prompt + "keep_ratio: ",
                                    "" + keep_ratio);
            WaveInterface.WriteLine(out, prompt + "horizontal_flip: ",
                                    "" + horizontal_flip);
        ...
        if (cnum_shot != 0)
        {
            if (UseDefaultShot())
                ...
                eval_shot = cnum_shot;
            WaveInterface.WriteLine(out, prompt + "num_expr: ",
                                    "" + num_waves / eval_shot);
        }
        else
        {
        ...
        {
            WaveInterface.WriteLine(out, prompt + "label" + "_" + exp_n + ": ",
                                    in_label[exp]);
            //add blank at the end of expression to fix bug when last expression character is \
            WaveInterface.WriteLine(out, prompt + "x_expr" + "_" + exp_n + ": ",
                                    AddNewLineCode(in_x[exp]));
            WaveInterface.WriteLine(out, prompt + "y_expr" + "_" + exp_n + ": ",
                                    AddNewLineCode(in_y[exp]));

            if (!is_image)
            {
                WaveInterface.WriteLine(out,
                                        prompt + "up_error" + "_" + exp_n + ": ",
                                        in_up_err[exp]);
                WaveInterface.WriteLine(out,
                ...
                for (sht = 0, sht_n = 1; sht < cnum_shot; sht++, sht_n++)
                {
                  //WaveInterface.WriteLine(out,prompt + "interpolate"+"_"+exp_n+"_"+sht_n+": ",""+interpolates[exp + sht]);
                    ...
                    WaveInterface.WriteLine(out,
                                            prompt + "mode_2D" + "_" + exp_n +
                                            "_" + sht_n + ": ",
                                            "" +
                                            mode2DCodeToString(mode2D[exp + sht]));
                    WaveInterface.WriteLine(out,
                                            prompt + "color" + "_" + exp_n + "_" +
                                            sht_n + ": ",
                                            "" + colors_idx[exp + sht]);
                    WaveInterface.WriteLine(out,
                                            prompt + "marker" + "_" + exp_n +
                                            "_" + sht_n + ": ",
                                            "" + markers[exp + sht]);
                    WaveInterface.WriteLine(out,
                                            prompt + "step_marker" + "_" + exp_n +
                                            "_" + sht_n + ": ",
    ...
    public void FromFile(Properties pr, String prompt, ColorMapDialog cmd) throws IOException
    {
        ...
        try
        {
            ...
            {
                show_legend = true;
                legend_x = Double.valueOf(prop.substring(prop.indexOf("(") + 1,
                    prop.indexOf(","))).doubleValue();
                legend_y = Double.valueOf(prop.substring(prop.indexOf(",") + 1,
                    prop.indexOf(")"))).doubleValue();
            }
                ...
                if (num_expr > 1)
                {
                    try
                    {
                        ...
                        while (st_y.hasMoreTokens())
                        {
                            ...
                            {
                                in_y[i] = "compile" +
                                    in_y[i].substring(in_y[i].indexOf("("));
                            }
                            i++;
                        }
                    }
            ...
            for (int idx = 1; idx <= num_expr; idx++)
            {
                ...
                for (int s = 1; s <= num_shot; s++)
                {
                    ...
                    prop = pr.getProperty(prompt + ".step_marker_" + idx + "_" +
                                          s);
                    if (prop != null)
                    {
                        try
