...
//of merging dimension arrays
	...
	{
		currSignalD = ((struct descriptor_signal **)signalsApd->pointer)[i];
		if(currSignalD->dimensions[0]->class != CLASS_R || currSignalD->dimensions[0]->dtype != DTYPE_RANGE)
	...
	else //All dimensions are expressed as range
	{
		...
		else
		{
			...
			for(i = 0; i < numSignals; i++)
			{
				...
				memcpy(&endingPtr[i*rangeD->ending->length], rangeD->ending->pointer, rangeD->ending->length);
				memcpy(&deltavalPtr[i*rangeD->deltaval->length], rangeD->deltaval->pointer, rangeD->deltaval->length);
			}
