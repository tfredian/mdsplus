...
static int TranslateName(char *name, int *highway, int *crate, int *slot)
{
  ...
  if (cnt != 3) {
    char tname[9];
    status = xlate_logical_name(name, tname);
    if (status)
      cnt = sscanf(tname, "ks%c%d:%d", &hwc, &c, &s);
  }
  if (cnt == 3) {
    hw=hwc-'a';
    status = 1;
                     ...
                     unsigned int num, void *data, int width, Iosb *iosb)
{
  ...
  printf("CamQrepw(%s, a=%d, f=%d,...) returning %d\n", name, a, f, status);
  return status;
}
                     ...
                     unsigned int num, void *data, int width, Iosb *iosb)
{
  ...
  printf("CamStopw(%s, a=%d, f=%d,...) returning %d\n", name, a, f, status);
  return status;
}
extern int CamQstopw(char *name, unsigned int a, unsigned int f,
                     unsigned int num, void *data, int width, Iosb *iosb)
{
  ...
  printf("CamQStopw(%s, a=%d, f=%d,...) returning %d\n", name, a, f, status);
  return status;
}
int main()
{
