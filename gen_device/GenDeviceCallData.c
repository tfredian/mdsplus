...
/*------------------------------------------------------------------------------
 ...
 Executable:								      */
    ...
    switch (mode) {
	case DevMODSTR :	
	    if (type != DTYPE_T) status = TdiData(record_d.pointer, setting_d_ptr MDS_END_ARG);	
	    else MdsCopyDxXd((struct descriptor *)&record_d, setting_d_ptr);
	    if (~status & 1) 
	    {
		if ((setting_d_ptr) && (setting_d_ptr->pointer)) setting_d_ptr->pointer->class = CLASS_D;
            }
            else
	    {
              char zero=0;
	...
	break;
	case DevMODINT :	
	    if (type != DTYPE_L) status = TdiLong(record_d.pointer, setting_d_ptr MDS_END_ARG);	
	    else MdsCopyDxXd((struct descriptor *)&record_d, setting_d_ptr);
	break;
	case DevMODFLO :	
	    if (type != DTYPE_F) status = TdiCvt(record_d.pointer, &float_d, setting_d_ptr MDS_END_ARG);	
	    else MdsCopyDxXd((struct descriptor *)&record_d, setting_d_ptr);
	break;
	case DevMODSLO :	
	...
	case DevMODRAN :	
		...
		if ((temp_xd.pointer)->dtype != DTYPE_RANGE) {
		  ...
		  DESCRIPTOR_RANGE(range_d, 0, 0, 0);
		    ...
		    status = TdiCvt(((struct descriptor_range *)temp_xd.pointer)->deltaval, &float_d, &delta_xd MDS_END_ARG);
	...
	case DevMODRANLON :	
		...
		if ((temp_xd.pointer)->dtype != DTYPE_RANGE) {
		  ...
		  EMPTYXD(delta_xd);
		    ...
		    status = TdiLong(((struct descriptor_range *)temp_xd.pointer)->deltaval, &delta_xd MDS_END_ARG);
		    range_d.begin = begin_xd.pointer;
		    range_d.ending = end_xd.pointer;
		    range_d.deltaval = delta_xd.pointer;
		    status = MdsCopyDxXd((struct descriptor *)&range_d, setting_d_ptr);
