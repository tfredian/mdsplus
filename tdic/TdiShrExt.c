...
#ifdef EXAMPLE
 dele *.exe;*,*.obj;*,*.opt;*
  mms/descrip=temp.mms;4 bpdmdsunix_axp.exe 
...
export DEBUG=-DDEBUG
make libBpdMdsUnix.so
cp libBpdMdsUnix.so /usr/lib/libBpdMdsUnix.so
ldconfig
#endif
...
/*  ================================================================================ */
struct descriptor_xd *rMdsValue(struct descriptor *expression, ...)  /**** NOTE: NULL terminated argument list expected ****/
{
   va_list incrmtr;
   int i,j;
   unsigned char nargs;
...
static int MdsToIp(struct descriptor **tdiarg ,short *len) {
   ...
   switch((*tdiarg)->dtype) {
    ...
    case DTYPE_MISSING:	/*      0		unspecified */
      dtype = 0;
      *len = 0; break;
...
/* if an array, print out other info */
   switch(desc->class) {
    case CLASS_S:		       /* fixed-length descriptor */
     printf("got descriptor [%d],[%d],[%d],[%lu]\n",desc->length,desc->dtype,desc->class,desc->pointer);
    ...
    case CLASS_A:		       /* Array descriptor */
      ...
      printf("aflags binscale[%d],redim[%d],column[%d],coeff[%d],bounds[%d]\n",
	     desca->aflags.binscale,desca->aflags.redim,desca->aflags.column,desca->aflags.coeff,desca->aflags.bounds);
      num = desca->arsize/desca->length;
      if(desca->aflags.coeff) {
	 int j;
	 array_bounds *bptr = (array_bounds *)desca;/* access the array bounds of descriptor_a */
