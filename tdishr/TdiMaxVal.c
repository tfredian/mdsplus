...
{ int *outp = (int *)out->pointer;\
  ...
  char testval;\
  int  status;\
  struct descriptor s_d={DESCRIPTOR_HEAD_INI(in->length,DTYPE_T,CLASS_S,0)}; \
  struct descriptor o_d={DESCRIPTOR_HEAD_INI(1,DTYPE_B,CLASS_S,0)};	\
  struct descriptor result={DESCRIPTOR_HEAD_INI(in->length,DTYPE_T,CLASS_S,0)}; \
