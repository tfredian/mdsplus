...
STATIC_CONSTANT descriptor_llength len = sizeof(int);
...
{
  struct descriptor unit_d = {DESCRIPTOR_HEAD_INI(0,DTYPE_T,CLASS_S,0)};
...
{
   int status;
   struct descriptor unit_d = {DESCRIPTOR_HEAD_INI(0,DTYPE_T,CLASS_D,0)};
...
STATIC_CONSTANT descriptor_llength length = 23;

	if (narg > 0 && list[0]) {
	  struct descriptor dtime = {DESCRIPTOR_HEAD_INI(sizeof(time),DTYPE_Q,CLASS_S,0)};
...
FILE	*unit;
struct descriptor_d dname = {DESCRIPTOR_HEAD_INI(0,DTYPE_T,CLASS_D,0)};
struct descriptor_d dmode = {DESCRIPTOR_HEAD_INI(0,DTYPE_T,CLASS_D,0)};
...
FILE	*unit = 0;
  ...
  if (status & 1) 
  {
    char line[4096];
    char *ans;
    struct descriptor line_d = {DESCRIPTOR_HEAD_INI( 0, DTYPE_T, CLASS_S, 0)};
