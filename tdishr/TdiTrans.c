...
unsigned char			out_dtype;
	...
	******************************************/
	if (status & 1) status = (*pfun->f2)(narg, uni, dat, cats, 0);
	...
	********************/
	if (pfun->f2 == Tdi2Mask2) {
		if (narg > 1 && cats[1].in_dtype != DTYPE_MISSING) pmask = dat[1].pointer;
	}
	else if (narg > 1 && cats[1].in_dtype != DTYPE_MISSING) {
		if (status & 1) status = TdiGetLong(dat[1].pointer, &dim);
		if (status & 1 && dim < 0) status = TdiBAD_INDEX;
	}
	else if (pfun->f2 == Tdi2Sign) status = TdiBAD_INDEX;

	if (pfun->f2 == Tdi2Mask3) {
		if (narg > 2 && cats[2].in_dtype != DTYPE_MISSING) pmask = dat[2].pointer;
	}
	else if (opcode == OpcReplicate || opcode == OpcSpread) {
		if (status & 1) status = TdiGetLong(dat[2].pointer, &ncopies);
	...
	**************************/
	...
	case CLASS_A :
		...
		if (rank > MAXDIM) status = TdiNDIM_OVER;
		/** Whole array. Unrestricted. Product of multipliers == size, unchecked **/
		else if (dim < 0) count_dim = (int)pa->arsize / (int)pa->length;
		/** Array with multipliers **/
		else if (pa->aflags.coeff) {
			if (dim < rank || dim <= rank && opcode == OpcSpread) {
				for (j = 0; j < dim; ++j) count_bef *=	pa->m[j];
				if (j < rank)		count_dim =	pa->m[j];
				for (; ++j < rank;)	count_aft *=	pa->m[j];
			}
			else status = TdiBAD_INDEX;
		}
		/** Simple vector. A mistake for some? **/
		else {
			j = (int)pa->arsize / (int)pa->length;
			if (dim == 1 && opcode == OpcSpread) count_bef = j;
			else if (dim > 0) status = TdiBAD_INDEX;
			else count_dim = j;
		}
		break;
	case CLASS_S :
	case CLASS_D :
		rank = 0;
		if (dim > 0 || dim > 1 && opcode == OpcSpread) status = TdiBAD_INDEX;
		break;
	default :
		status = TdiINVCLADSC;
		break;
	...
	if (status & 1) N_ELEMENTS(pmask, mul);
	if (status & 1) {
		if (pmask->class == CLASS_A && mul < count_bef * count_dim * count_aft) status = TdiMISMATCH;
		step_dim = count_bef;
	...
	****************************************/
	...
	Act on data.
	***********/
	status = (*pfun->f3)(pa, pmask, pd, count_dim, count_bef, count_aft, step_dim, step_bef, step_aft);
	if (!(status & 1)) goto err;
	...
	*******************************/
	...
	else if (opcode == OpcAccumulate) dat[0] = EMPTY_XD;
	else if (opcode == OpcProduct) MdsFree1Dx(&uni[0], NULL);
