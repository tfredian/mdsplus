...
int				nid, *pnid;
	...
	*****************************/
	if (list[0] == 0) return MdsCopyDxXd(&missing, out_ptr);
	...
	case CLASS_XD :
		...
		***************************************************/
		switch(list[0]->dtype) {
		case DTYPE_DSC :
			if(list[0]->pointer == 0) return TdiNULL_PTR;
...
*****************************************************************************/
	...
	case CLASS_XS :
		...
		case DTYPE_PATH :
			...
			break;
		default :
			if (list[0]->dtype < 160)	status = -1;
			else status = TdiINVCLADTY;
			break;
		}
		break;
	case CLASS_R :
		...
		default :
			...
			***********************/
			if (list[0]->dtype < DTYPE_PARAM) status = TdiINVCLADTY;
			else status = -1;
			break;
		}
		break;
	...
	****************************/
	if (status == -1) status = MdsCopyDxXd(list[0], out_ptr);
	if (list[0]->pointer) MdsFree1Dx((struct descriptor_xd *)list[0], NULL);
	return status;
}
