...
#endif	
 struct descriptor dname = DESCRIPTOR_INIT(0,DTYPE_T,CLASS_S,0);
int	status;
...
STATIC_ROUTINE void StoreAnswer(int idx, struct descriptor *dst, int type)
{
	...
	EMPTYXD(xd);
	struct descriptor t_dsc = DESCRIPTOR_INIT(0, DTYPE_T, CLASS_S, 0 );
...
int 	rblob;
{
	...
	struct descriptor_d 	madeup = DESCRIPTOR_INIT(0,DTYPE_T,CLASS_D,0);
	char				*buf;
	static struct descriptor name = DESCRIPTOR_INIT(0,DTYPE_T,CLASS_S,0);
...
int		rows = 0;
ARGLIST		user_args = {0};
struct descriptor dtext = DESCRIPTOR_INIT(0,DTYPE_T,CLASS_D,0);
struct descriptor dq_text = DESCRIPTOR_INIT(0,DTYPE_T,CLASS_D,0);
struct descriptor drows = DESCRIPTOR_INIT(sizeof(rows),DTYPE_L,CLASS_S,0);
STATIC_CONSTANT DESCRIPTOR(zero, "\0");
	...
	if (status & 1) {
	  status = MdsCopyDxXd(&drows, out_ptr);
          ...
          struct descriptor msg = DESCRIPTOR_INIT(0, DTYPE_T, CLASS_S, 0);
