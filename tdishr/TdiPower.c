...
STATIC_CONSTANT struct descriptor one_dsc = {DESCRIPTOR_HEAD_INI(sizeof(one),DTYPE_L,CLASS_S,(char *)&one)};
...
struct descriptor	duno;
	...
	*******************************************/
	if (y->dtype != DTYPE_L) {
		...
		else	status = Tdi3Log(x, z);
		if (status & 1) status = Tdi3Multiply(y, z, z);
		if (status & 1) status = Tdi3Exp(z, z);
	}
	/******************************************
	...
	******************************************/
		...
		if ((yy = *(int *)y->pointer) <= 0) {
			if (yy == 0) return TdiConvert(&one_dsc, z);
			yy = -yy;
			duno = *x;
			duno.pointer = uno;
			status = TdiConvert(&one_dsc, &duno);
			if (status & 1) status = Tdi3Divide(&duno, x, x);
		}
		for (; !(yy & 1) && status & 1; yy >>= 1) status = Tdi3Multiply(x, x, x);
		if (x->class != CLASS_A)
			_MOVC3(z->length, x->pointer, z->pointer);
		else _MOVC3(z->arsize, x->pointer, z->pointer);
		for (; (yy >>= 1) > 0 && status & 1;) {
			status = Tdi3Multiply(x, x, x);
			if (yy & 1 && status & 1) status = Tdi3Multiply(x, z, z);
		}
	}
	/************************
	...
	************************/
	...
	char	xx[32];
	struct descriptor dx = {DESCRIPTOR_HEAD_INI(0,0,CLASS_S,0)};
	char	*px = x->pointer;
	...
	int	n;
		...
		duno = dx;
		duno.pointer = uno;
		if (status & 1) status = TdiConvert(&one_dsc, &duno);
		for (; --n >= 0 && status & 1; px += incx, dz.pointer += incz) {
			_MOVC3(dx.length, px, xx);
			if ((int)(yy = *py++) <= 0) {
				if (yy == 0) {_MOVC3(dz.length, uno, dz.pointer); continue;}
				yy = -yy;
				status = Tdi3Divide(&duno, &dx, &dx);
			}
			for (; !(yy & 1) && status & 1; yy >>= 1) status = Tdi3Multiply(&dx, &dx, &dx);
			_MOVC3(dz.length, dx.pointer, dz.pointer);
			for (; (yy >>= 1) > 0 && status & 1;) {
				status = Tdi3Multiply(&dx, &dx, &dx);
				if (yy & 1 && status & 1) status = Tdi3Multiply(&dx, &dz, &dz);
			}
		}
	}
	return status;
...
int Tdi3Merge(struct descriptor_a *pdtrue, struct descriptor_a *pdfalse, struct descriptor_a *pdmask,struct descriptor_a *pdout)
{
  ...
  lquad    *pq;
	...
	case 1 : for (pc = (char *)po; --n >= 0; pm+=stepm, pt+=stept, pf += stepf)
			if (*pm & 1)	*pc++ = *(char *)pt;
			else		*pc++ = *(char *)pf;
		break;
	case 2 : for (ps = (short *)po; --n >= 0; pm+=stepm, pt+=stept, pf += stepf)
			if (*pm & 1)	*ps++ = *(short *)pt;
			else		*ps++ = *(short *)pf;
		break;
	case 4 : for (pl = (int *)po; --n >= 0; pm+=stepm, pt+=stept, pf += stepf)
			if (*pm & 1)	*pl++ = *(int *)pt;
			else		*pl++ = *(int *)pf;
		break;
	case 8 : for (pq = (lquad *)po; --n >= 0; pm+=stepm, pt+=stept, pf += stepf)
			if (*pm & 1)	*pq++ = *(lquad *)pt;
			else		*pq++ = *(lquad *)pf;
		break;
	default :
		{int sm = stepm, st = stept, sf = stepf;
			for (; --n >= 0; pm+=sm, pt+=st, pf += sf, po+=len)
				if (*pm & 1)	_MOVC3(len, pt, po);
				else		_MOVC3(len, pf, po);
		}
		break;
	}
	return status;
