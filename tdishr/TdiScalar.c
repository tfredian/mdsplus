...
int				cmode = 0, j, (*routine)() = fun_ptr->f3;
	...
	******************************************/
	if (status & 1) status = (*fun_ptr->f2)(narg, uni, dat, cats, &routine);
	...
	******************************/
	...
	Find correct shape.
	******************/
	if (status & 1) status = TdiGetShape(0, dat, 0, cats[narg].out_dtype, &cmode, out_ptr);

	/*****************************
	Act on data, linear arguments.
	*****************************/
	if (status & 1) switch (fun_ptr->m2) {
...
int			status = 1, n;
	...
	switch (in_ptr->dtype) {
	default : n = 0; status = TdiINVDTYDSC; break;
	case DTYPE_B :
	case DTYPE_W :
	case DTYPE_L :
	case DTYPE_Q :
	case DTYPE_O :
		n = 8 * in_ptr->length - 1; break;
	case DTYPE_BU :
	case DTYPE_WU :
	case DTYPE_LU :
	case DTYPE_QU :
	case DTYPE_OU :
		n = 8 * in_ptr->length; break;
	case DTYPE_FC : case DTYPE_F : n = 24; break;
	case DTYPE_FSC : case DTYPE_FS : n = 24; break;
	case DTYPE_DC : case DTYPE_D : n = 56; break;
	case DTYPE_GC : case DTYPE_G : n = 53; break;
	case DTYPE_FTC : case DTYPE_FT : n = 53; break;
	}
...
struct descriptor	*out_ptr)
{
       int			status = 1;
       double digits_d;
       struct descriptor digits = {DESCRIPTOR_HEAD_INI(sizeof(digits_d),DTYPE_NATIVE_DOUBLE,CLASS_S,0)};
       digits.pointer = (char *)&digits_d;
       status = TdiDigits(x_ptr,&digits MDS_END_ARG);
       if (status & 1)
       {
	 STATIC_CONSTANT double two_d = 2.;
	 STATIC_CONSTANT struct descriptor two = {DESCRIPTOR_HEAD_INI(sizeof(two_d),DTYPE_NATIVE_DOUBLE,CLASS_S,(char *)&two_d)};
	 digits_d = 1. - digits_d;
...
int			status = 1, n;
	...
	switch (in_ptr->dtype) {
	default : n = 0; status = TdiINVDTYDSC; break;
	case DTYPE_FS : case DTYPE_FSC : case DTYPE_FC : case DTYPE_F : n = 127; break;
	case DTYPE_DC : case DTYPE_D : n = 127; break;
	case DTYPE_FT : case DTYPE_FTC : case DTYPE_GC : case DTYPE_G : n = 1023; break;
	}
	*(int *)out_ptr->pointer = n;
	return status;
}
...
int			status = 1, n;
	...
	switch (in_ptr->dtype) {
	default : n = 0; status = TdiINVDTYDSC; break;
	case DTYPE_FS : case DTYPE_FSC : case DTYPE_FC : case DTYPE_F : n = (int)(_factor * 23); break;
	case DTYPE_DC : case DTYPE_D : n = (int)(_factor * 55); break;
	case DTYPE_FT : case DTYPE_FTC : case DTYPE_GC : case DTYPE_G : n = (int)(_factor * 52); break;
	}
	*(int *)out_ptr->pointer = n;
	return status;
}
...
int			n, status = 1;
	...
	case DTYPE_FS : case DTYPE_FT :
	case DTYPE_FSC : case DTYPE_FTC :
		n = 2;
	}
	*(int *)out_ptr->pointer = n;
	return status;
...
int			status = 1, n;
	...
	switch (in_ptr->dtype) {
	default : n = 0; status = TdiINVDTYDSC; break;
	case DTYPE_FS : case DTYPE_FSC : case DTYPE_FC : case DTYPE_F : n = (int)(_factor * 127); break;
	case DTYPE_DC : case DTYPE_D : n = (int)(_factor * 127); break;
	case DTYPE_FT : case DTYPE_FTC : case DTYPE_GC : case DTYPE_G : n = (int)(_factor * 1023); break;
	}
	*(int *)out_ptr->pointer = n;
	return status;
}
...
int			n;
	...
	if (arr_ptr->class == CLASS_A) n = arr_ptr->aflags.coeff ? arr_ptr->dimct : 1;
	else if (arr_ptr->dtype == DTYPE_MISSING) n = -1;
	else n = 0;
	*(int *)out_ptr->pointer = n;
	return 1;
}
...
*/
int			Tdi3SelectedIntKind(
struct descriptor	*pdr,
struct descriptor	*pout)
{
int	range, status;
float	frange;
	...
	if (status & 1) {
		...
		if	(frange <= 07) range = DTYPE_B;
		else if (frange <= 15) range = DTYPE_W;
		else if (frange <= 31) range = DTYPE_L;
		else if (frange <= 63) range = DTYPE_Q;
		else if (frange <= 127) range = DTYPE_O;
		else range = -1;
...
*/
int			Tdi3SelectedRealKind(
struct descriptor	*pdp,
struct descriptor	*pdr,
struct descriptor	*pout)
{
...
float	fprec, frange;
	...
	if (status & 1) status = TdiGetLong(pdr, &range);
	if (status & 1) {
		fprec = prec / _factor;
		frange = range / _factor;
...
int			status = 1;
	...
	switch (in_ptr->dtype) {
	default : status = TdiINVDTYDSC; break;
	case DTYPE_F : case DTYPE_FC : memcpy(out_ptr->pointer,F_TINY,sizeof(F_TINY)); break;
	case DTYPE_FS : case DTYPE_FSC : memcpy(out_ptr->pointer,FS_TINY,sizeof(FS_TINY)); break;
	case DTYPE_D : case DTYPE_DC : memcpy(out_ptr->pointer,D_TINY,sizeof(D_TINY)); break;
	case DTYPE_G : case DTYPE_GC : memcpy(out_ptr->pointer,G_TINY,sizeof(G_TINY)); break;
	case DTYPE_FT : case DTYPE_FTC : memcpy(out_ptr->pointer,FT_TINY,sizeof(FT_TINY)); break;
	}
	return status;
}
/*---------------------------------------------------
...
*/
int			Tdi3DotProduct(
struct descriptor_a	*in1_ptr,
struct descriptor_a	*in2_ptr,
struct descriptor	*out_ptr)
{
...
EMPTYXD(tmp);
	...
	case DTYPE_B : case DTYPE_BU:
		for (lsum = 0; --n >= 0; p1+=inc1, p2 += inc2) lsum += *(char *)p1 * *(char *)p2;
		*(char *)out_ptr->pointer = (char)lsum;
		break;
	case DTYPE_W : case DTYPE_WU:
		for (lsum = 0; --n >= 0; p1+=inc1, p2 += inc2) lsum += *(short *)p1 * *(short *)p2;
		*(short *)out_ptr->pointer = (short)lsum;
		break;
	case DTYPE_L : case DTYPE_LU:
		for (lsum = 0; --n >= 0; p1+=inc1, p2 += inc2) lsum += *(int *)p1 * *(int *)p2;
	...
	default :
		if (inc1) {
			...
			if (status & 1) status = TdiSum(in1_ptr, &tmp MDS_END_ARG);
		}
		else {
			status = Tdi3Multiply(in1_ptr, in2_ptr, in2_ptr);
			if (status & 1) status = TdiSum(in2_ptr, &tmp MDS_END_ARG);
		}
		if (status & 1) _MOVC3(out_ptr->length, tmp.pointer->pointer, out_ptr->pointer);
		break;
	}
        MdsFree1Dx(&tmp,0);
	return status;
