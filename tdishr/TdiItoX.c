...
STATIC_CONSTANT struct descriptor dmbig = {DESCRIPTOR_HEAD_INI(sizeof(mbig),DTYPE_F,CLASS_S,(char *)&mbig)};
STATIC_CONSTANT struct descriptor dbig = {DESCRIPTOR_HEAD_INI(sizeof(big),DTYPE_F,CLASS_S,(char *)&big)};
STATIC_CONSTANT struct descriptor dmhuge = {DESCRIPTOR_HEAD_INI(sizeof(mhuge),DTYPE_L,CLASS_S,(char *)&mhuge)};
STATIC_CONSTANT struct descriptor dhuge = {DESCRIPTOR_HEAD_INI(sizeof(lhuge),DTYPE_L,CLASS_S,(char *)&lhuge)};
STATIC_CONSTANT struct descriptor dmone = {DESCRIPTOR_HEAD_INI(sizeof(mone),DTYPE_L,CLASS_S,(char *)&mone)};
...
struct descriptor		dk0 = {DESCRIPTOR_HEAD_INI(sizeof(int),DTYPE_L,CLASS_S,0)};
struct descriptor		dk1 = {DESCRIPTOR_HEAD_INI(sizeof(int),DTYPE_L,CLASS_S,0)};
...
************************************************************/
if (!flag) status = TdiGetData(&omits[1], list[0], &dimen);
 else {
   ...
   if (status & 1 && dimen.pointer->dtype == DTYPE_WITH_UNITS) {
     ...
     if (status & 1) status = TdiGetData(&omits[1], &dimen, &dimen);
   }
 }
if (status & 1) switch (dimen.pointer->dtype) {
    /*********************
  ...
  case DTYPE_DIMENSION :
    ...
    if (status & 1 && pwin && pwin->dtype == DTYPE_WITH_UNITS) {
      ...
      if (status & 1) status = MdsCopyDxXd((struct descriptor *)&tmp, &window);
    }
    if (status & 1) status = TdiEvaluate(pdim->axis, &axis MDS_END_ARG);
    ...
    if (status & 1 && paxis && paxis->dtype == DTYPE_WITH_UNITS) {
      ...
      if (status & 1) status = MdsCopyDxXd((struct descriptor *)&tmp, &axis);
    }
    paxis = (struct descriptor_range *)axis.pointer;
    pwin = (struct descriptor_window *)window.pointer;
    break;
...
if (status & 1 && pwin && pwin->startidx)		status = TdiGetLong(pwin->startidx, &k0);
 else k0 = -HUGE;
if (status & 1 && pwin && pwin->endingidx)		status = TdiGetLong(pwin->endingidx, &k1);
 else k1 = HUGE;
if (status & 1)	/*bug 2/26/91 include status test both ways*/
  if (pwin && pwin->value_at_idx0)			status = TdiData(pwin->value_at_idx0, &xat0 MDS_END_ARG);
  else							status = TdiData(0, &xat0 MDS_END_ARG);

if (status & 1) switch (paxis->dtype) {
  case DTYPE_SLOPE :
    pslope = (struct descriptor_slope *)paxis;
    paxis = &fake;
    fake = fake0;
    switch (pslope->ndesc) {
    case 3 : fake.ending = pslope->segment[0].ending;
    case 2 : fake.begin = pslope->segment[0].begin;
    case 1 : fake.deltaval = pslope->segment[0].slope;
      break;
    case 0 : status = TdiINVDTYDSC; break;
    default :
  ...
  case DTYPE_RANGE :
    ...
    if (status & 1) {	/*bug 2/26/91 include status set in bracket*/
      for (j1 = nran; --j1 >= 0;) if (dat[j1].pointer->class == CLASS_A) {
	  jseg = ((struct descriptor_a *)dat[j1].pointer)->arsize
	    / (int)dat[j1].pointer->length;
	  if (nseg > jseg) nseg = jseg;
	}
      if (nseg == HUGE) nseg = 1;
      if (nseg <= 0) status = TdiNULL_PTR;
    }
    ...
    if (nseg == 1) {
      ...
      if (tslo) dat[delta].pointer->class = CLASS_S;
      if (big_beg && big_end) goto select;
      if (big_beg || big_end) goto counted;
    }
    status = TdiCvtArgs(nran, dat, cats);
    f = TdiFaultClear(0);
    if (status & 1) status = TdiSubtract(dat[end].pointer, dat[beg].pointer, &cnt MDS_END_ARG);
    if (tslo && status & 1) status = TdiDivide(&cnt, dat[delta].pointer, &cnt MDS_END_ARG);
    if (!(TdiFaultClear(0) & 1) && status & 1) status = TdiFixRoprand(&cnt, &dhuge, &cnt);
    if (status & 1)	status = TdiDim(&cnt, &dmone, &cnt MDS_END_ARG);
    if (status & 1) status = TdiMin(&cnt, &dhuge, &cnt MDS_END_ARG);
    if (status & 1 && cnt.pointer->dtype != DTYPE_L) status = TdiNint(&cnt, &cnt MDS_END_ARG);
  ...
  counted:
    kseg = 0;
    if (nseg > 1 || !big_beg) {
      if (status & 1) status = TdiSubtract(dat[beg].pointer, xat0.pointer, &tmp MDS_END_ARG);
      if (tslo && status & 1) status = TdiDivide(&tmp, dat[delta].pointer, &tmp MDS_END_ARG);
      if (!(TdiFaultClear(0) & 1) && status & 1) status = TdiFixRoprand(&tmp, &dmhuge, &tmp);
    }
    left = -HUGE;
    if (!pwin) left = 0;	/*bug 2/26/91 missing window*/
    else if (nseg > 1) {
      if (status & 1) status = TdiGt(tmp.pointer, 0, out_ptr MDS_END_ARG);
      if (status & 1) {
	...
	if (tmp.pointer->dtype == DTYPE_L)
	  ...
	  if (status & 1) status = TdiMax(&uni1, &dmhuge, &tmp MDS_END_ARG);
	  MdsFree1Dx(&uni1, NULL);
	  if (status & 1) status = TdiMin(&tmp, &dhuge, &tmp MDS_END_ARG);
	  if (status & 1) status = TdiGetLong(tmp.pointer, &left);
	}
      if (kseg == 0 && left > 0) left = 0; /* 1/7/92 trig before clock, use first time */
      if (nseg == 1) {
	if (k0 < left) k0 = left;
	if (left <= -HUGE) big_beg = 1;
	else {
	  int1 = dk0;
	  int1.pointer = (char *)&left;
	  if (status & 1) status = MdsCopyDxXd(&int1, &tmp);
	  if (tslo && status & 1) 
	    status = TdiMultiply(&tmp, dat[delta].pointer, &tmp MDS_END_ARG);
	  if (status & 1) status = TdiSubtract(&dat[beg], &tmp, &xat0 MDS_END_ARG);
	  if (!big_end) right = left + *(int *)cnt.pointer->pointer;
	}
      }
    }
    ...
    if (nseg > 1 && status & 1) {
      ...
      for (jseg = kseg; --jseg >= 0;) left -= *ptest++; /*bug 2/26/91 increment ptest*/
      if (k0 < left) k0 = left;
      if (left <= -HUGE) {
	big_beg = 1;
	right = left + HUGE;
	*pcnt = 1;
	_MOVC3(	dat[end].pointer->length,
		dat[end].pointer->pointer,
		dat[beg].pointer->pointer);
      }
      else right = left + *pcnt;
    }
    ...
    *****************************************************************/
	...
	if (tmp.pointer->dtype == DTYPE_L)
	  ...
	  if (!(TdiFaultClear(0) & 1)) status = TdiFixRoprand(&tmp, &dhuge, &tmp);
	  if (status & 1) status = TdiFloor(&tmp, &tmp MDS_END_ARG);
	  if (status & 1) status = TdiMax(&tmp, &dmhuge, &tmp MDS_END_ARG);
	  if (status & 1) status = TdiMin(&tmp, &dhuge, &tmp MDS_END_ARG);
	  if (status & 1) status = TdiGetLong(&tmp, &right);
	}
      }
	...
	if (status & 1) status = MdsCopyDxXd(&int1, &tmp);
	if (tslo && status & 1) status = TdiMultiply(&tmp, &del1, &tmp MDS_END_ARG);
	if (status & 1) status = TdiSubtract(&tst1, &tmp, &xat0 MDS_END_ARG);
      }
      ++right;
    }
    ...
    if (nseg > 1) {
      *pcnt = right - *pcnt;
      for (jseg = nseg; --jseg > 0;) j1 = *++pcnt, *pcnt = right, right += j1;
    }
    if (!big_end && k1 >= right) k1 = right - 1;
  ...
  select:
	...
	if (!flag && rptr->dtype == DTYPE_RANGE
	    && (rptr->ndesc == 2 || rptr->ndesc == 3 && rptr->deltaval == 0)) {
     ...
     ******************************************************************/
    if (nseg > 1 && (flag || arg1)) {
      if (status & 1) status = TdiBsearch(out_ptr->pointer,
	...
	status = TdiMap(&dat[delta], &tmp, &dat[delta] MDS_END_ARG);
      if (tslo && status & 1) status = TdiMultiply(&cnt, dat[delta].pointer, &cnt MDS_END_ARG);
      if (status & 1) status = TdiSubtract(&dat[beg], &cnt, &xat0 MDS_END_ARG);
    }
    
    if (flag) {
      if (tslo && status & 1) status = TdiMultiply(out_ptr, &dat[delta], out_ptr MDS_END_ARG);
      if (status & 1) status = TdiAdd(out_ptr, &xat0, out_ptr MDS_END_ARG);
      /*	if (nseg > 1 && status & 1) status = TdiMin(out_ptr, &dat[end], out_ptr MDS_END_ARG);*/
    }
    else if (arg1) {
      /*	if (nseg > 1 && status & 1) status = TdiMin(out_ptr, &dat[end], out_ptr MDS_END_ARG);*/
      if (status & 1) status = TdiSubtract(out_ptr, &xat0, out_ptr MDS_END_ARG);
      if (tslo && status & 1) status = TdiDivide(out_ptr, &dat[delta], out_ptr MDS_END_ARG);
    }
    MdsFree1Dx(&cnt, NULL);
    MdsFree1Dx(&uni1, NULL);
    ...
    ********************************/
    if (status & 1) status = TdiMasterData(narg > 1, &sig1, &units, &cmode, out_ptr);
  ...
  plain:		status = TdiData(&axis, &axis MDS_END_ARG);
    ...
    j1 = paxis->length;
    if (status & 1)	status = TdiSort(paxis, &tmp MDS_END_ARG);
    if ((pwin || flag || arg1) && status & 1) {
      status = TdiMap(paxis, tmp.pointer, &window MDS_END_ARG);
    }
    if (pwin) {
      if (status & 1) status = TdiBsearch(&xat0, paxis, pmode, &xat0 MDS_END_ARG);
      if (status & 1) status = TdiGetLong(&xat0, &left);
      if (status & 1) N_ELEMENTS(paxis, right);
      left = -left;
      right += left - 1;
      if (k0 < left) k0 = left;
      if (k1 > right) k1 = right;
    }
    else {
      if (status & 1) status = TdiLbound(axis.pointer, &dk0 MDS_END_ARG);
      if (status & 1) status = TdiUbound(axis.pointer, &dk1 MDS_END_ARG);
    }
    if (status & 1)
      if (special) {
	if (flag) {
	  ...
	  if (status & 1) {
	    ...
	    _MOVC3(j1, (char *)window.pointer->pointer+j1*(k1-k0), out_ptr->pointer->pointer+j1);
	  }
	}
	else {
	  int	limits[2];
	...
	if (flag) status = MdsCopyDxXd((struct descriptor *)&axis, out_ptr);	/*unsorted values*/
    ...
    if (status & 1) status = TdiMasterData(0, &sig1, &units, &cmode, out_ptr);
    break;
  }
MdsFree1Dx(&tmp, NULL);
