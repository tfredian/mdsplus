...
#define DATUM(type, x, data) \
	...
	STATIC_CONSTANT struct descriptor	Tdi##x##Constant = {DESCRIPTOR_HEAD_INI(sizeof(type), DTYPE_##type, CLASS_S, (char *)&d##x)}; \
	struct descriptor *Tdi3##x(){return &Tdi##x##Constant;}

#define DERR(type, x, data, error) \
	STATIC_CONSTANT type			d##x = data;\
        STATIC_CONSTANT type			e##x = error;\
	STATIC_CONSTANT struct descriptor	dd##x = {DESCRIPTOR_HEAD_INI(sizeof(type), DTYPE_##type, CLASS_S, (char *)&d##x)}; \
	STATIC_CONSTANT struct descriptor	de##x = {DESCRIPTOR_HEAD_INI(sizeof(type), DTYPE_##type, CLASS_S, (char *)&e##x)}; \
