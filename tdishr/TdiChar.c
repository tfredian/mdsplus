...
int	step = out_ptr->length, status = 1, n;
	...
	if (status & 1) for (; --n >= 0;) {
		pe1 += step;
		pe2 += step;
		while (ps1 < pe1 && (*ps1 == ' ' || *ps1 == '\t')) ps1++;
		while (ps1 < pe1) *ps2++ = *ps1++;
		while (ps2 < pe2) *ps2++ = ' ';
	}
	return status;
}
...
int	step = out_ptr->length, status = 1, n, last;
	...
	N_ELEMENTS(out_ptr, n);
	if (status & 1) for (last = n*step - 1, pe1 = ps1 += last, pe2 = ps2 += last; --n >= 0;) {
		ps1 -= step;
		ps2 -= step;
		while (ps1 < pe1 && (*pe1 == ' ' || *pe1 == '\t')) pe1--;
		while (ps1 < pe1) *pe2-- = *pe1--;
		while (ps2 < pe2) *pe2-- = ' ';
	}
	return status;
}
...
int	status = 1, n;
	...
	N_ELEMENTS(out_ptr, n);
	if (status & 1) for (; --n >= 0; p1 += step1, p2 += step2) {
		for (j1 = size1, pt1 = p1; --j1 >= 0;) *p3++ = *pt1++;
		for (j2 = size2, pt2 = p2; --j2 >= 0;) *p3++ = *pt2++;
	}
	return status;
}
/*------------------------------------------------------------------------------
...
*/
int			Tdi3Element(
struct descriptor	*number_ptr,
struct descriptor	*delim_ptr,
struct descriptor	*source_ptr,
struct descriptor	*out_ptr)
{
...
*/
int			Tdi3Index(
struct descriptor	*str_ptr,
struct descriptor	*sub_ptr,
struct descriptor	*bac_ptr,
struct descriptor	*out_ptr)
{
	...
	while (--n >= 0) {
		...
		int	m, limit = str_dsc.length - sub_dsc.length;
			for (m = 0; j = m - 1, m++ <= limit;)
				if ((m = StrPosition(&str_dsc, &sub_dsc, &m)) <= 0) break;
		}
		else j = StrPosition(&str_dsc, &sub_dsc, 0) - 1;
		*out_point++ = j;
		...
		if (*bac_dsc.pointer & 1) {
			...
			********************************************************************/
			...
			while (--tmp_dsc.pointer >= str_dsc.pointer)
				if (StrPosition(&set_dsc, &tmp_dsc, 0) != 0) break;
			j = tmp_dsc.pointer - str_dsc.pointer;
		}
		else	j = StrFindFirstInSet(&str_dsc, &set_dsc) - 1;
		*out_point++ = j;
		str_dsc.pointer += str_step;
...
struct descriptor_d	one_dsc = {0,DTYPE_T,CLASS_D,0};
	...
	for (; --n >= 0; tmp_dsc.pointer += step) {
		...
		****************************************************/
		...
		if (status & 1) status = StrTrim(&one_dsc, &one_dsc, 0);
		if (!(status & 1)) break;
		/******************************
		...
		******************************/
		...
		if (len > 4 && strncmp(str_ptr, "Opc", 4) == 0) {str_ptr += 4; len -= 4;}
...
int	*out_point = (int *)out_ptr->pointer;
	...
	while (--n >= 0) {
		...
		else	j = StrFindFirstNotInSet(&str_dsc, &set_dsc) - 1;
		*out_point++ = j;
