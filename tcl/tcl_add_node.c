...
int   TclAddNode()		/* Return: status			*/
   {
    ...
    if (cli_get_value("MODEL",&dsc_modelType) & 1)
       {
        ...
        sts = TreeAddConglom(dsc_nodnam.pointer,
                            dsc_modelType.pointer,&nid);
       }
    else
       {
        ...
        sscanf(dsc_usageStr.pointer,"%d",&k);
        usage = (char)k;
        sts = TreeAddNode(dsc_nodnam.pointer,&nid,usage);
       }
    ...
    if (sts & 1)
       ...
       {
        MdsMsg(sts,"Error adding node %s",dsc_nodnam.pointer);
#ifdef vms
        lib$signal(sts,0);
#endif
       }
